...
- (void)viewDidLoad {
    ...
    //_doneButton.backgroundColor = kPurpleColor;
    ...
    if(!_doneButtonImage) {
        ...
        _doneButton.frame = CGRectMake(kScreenW-42, 40, 40, 25);
...
- (CGRect)frameForDoneButtonAtOrientation:(UIInterfaceOrientation)orientation {
    ...
    if (_doneButtonImage != nil) {
        //return CGRectMake(kScreenW-32, 30, _doneButtonImage.size.width, _doneButtonImage.size.height);
        return CGRectMake(kScreenW-42, 40, 40, 25);
    }else{
        //return CGRectMake(kScreenW-32, 40, 22, 5);
        return CGRectMake(kScreenW-42, 40, 40, 25);
    }
}
...
//    if (_senderViewForAnimation && _currentPageIndex == _initalPageIndex)
//    {
        [self getActionIndex:(int)_currentPageIndex];
       // IDMZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
       // [self performCloseAnimationWithScrollView:scrollView];
//    }
//    else {
...
#pragma mark保存图片到本地
- (void)getActionIndex:(int)imageIndex
{
    ...
    {
     [headImgSheet addButtonWithTitle:@"保存"];
    }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger) buttonIndex
{
    if (buttonIndex == 0)
    {
       // [self saveImageViewToPhone];
        if (_currentPageIndex < _photos.count)
        {
            if (self.delegate)
            {
                [[NSNotificationCenter defaultCenter]postNotificationName:@"deleteImgView" object:[NSString stringWithFormat:@"%d",(int)_currentPageIndex]];
                //[[NSNotificationCenter defaultCenter] postNotificationName:@"deleteImgView" object:nil];
            ...
            //[self updateToolbar];
//            CGRect pageFrame = [self frameForPageAtIndex:_currentPageIndex+1];
//            [_pagingScrollView setContentOffset:CGPointMake(pageFrame.origin.x - PADDING-kScreenW, 0) animated:YES];
//            [self updateToolbar];
...
- (void)saveImageViewToPhone
{
    IDMPhoto *photo =_photos[_currentPageIndex];
    UIImageView *imageView = [[UIImageView alloc]init];
    [imageView sd_setImageWithURL:photo.photoURL placeholderImage:kDefaultPreloadHeadImg];
...
{
    if (error == nil) {
       [[FWHUDHelper sharedInstance] tipMessage:@"保存成功"];
        
    }else{
      [[FWHUDHelper sharedInstance] tipMessage:@"保存失败"];
    }
}
