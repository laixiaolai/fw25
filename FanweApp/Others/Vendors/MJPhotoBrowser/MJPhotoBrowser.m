...
- (void)loadImageNearIndex:(int)index
{
    if (index > 0) {
        ...
        __block MJPhoto *photo = _photos[index - 1];
        
        [[SDWebImageManager sharedManager].imageDownloader downloadImageWithURL:photo.url options:SDWebImageLowPriority|SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
