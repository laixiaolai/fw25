    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns(line)) {
            ...
            if (imageName) {
                ...
                if ([imageName isEqualToString:@"Expression_1000"] && self.customEmojiImage)
                {
                    UIImage *image = self.customEmojiImage;
                    runBounds.origin.y -= emojiOriginYOffset; //稍微矫正下。
                    CGContextDrawImage(c, runBounds, image.CGImage);
                }
                else if ([imageName isEqualToString:@"Expression_1002"] && self.customImage)
                {
                    UIImage *image = self.customImage;
                    runBounds.origin.y -= emojiOriginYOffset; //稍微矫正下。
                    runBounds.size.width -= 5;
                    runBounds.size.height -= 5;
                    CGContextDrawImage(c, runBounds, image.CGImage);
                }
                else
                {
