...
#define kRepeatCount    1 // 循环次数

NSString * const MYID = @"cycleCell2";

@interface SDCycleScrollView2 () <UICollectionViewDataSource, UICollectionViewDelegate>
...
@implementation SDCycleScrollView2
...
{
    SDCycleScrollView2 *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)setupMainView
{
    ...
    mainView.bounces = NO;
    [mainView registerClass:[SDCollectionViewCell class] forCellWithReuseIdentifier:MYID];
...
- (void)setImagesGroup:(NSMutableArray *)imagesGroup
{
    if ([imagesGroup count]>0) {
        ...
        _totalItemsCount = imagesGroup.count * kRepeatCount;
...
{
    SDCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:MYID forIndexPath:indexPath];
    ...
    [cell addSubview:self.imagesGroup[itemIndex]];
