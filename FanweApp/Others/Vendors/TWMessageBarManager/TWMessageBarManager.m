...
CGFloat const kTWMessageBarStyleSheetMessageBarAlpha = 1.0f;
...
- (void)showNextMessage
{
    if ([self.messageBarQueue count] > 0)
    {
        ...
        UIPanGestureRecognizer *gest2 = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(itemSelected:)];
        [messageView addGestureRecognizer:gest2];
...
- (TWMessageBarViewController *)messageBarViewController
{
    if (!self.messageWindow)
    {
        ...
        self.messageWindow.windowLevel = UIWindowLevelStatusBar-1;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self.delegate respondsToSelector:@selector(styleSheetForMessageView:)])
    {
        ...
        if ([[UIDevice currentDevice] tw_isRunningiOS7OrLater])
        {
            ...
            {
                [self.titleString drawWithRect:CGRectMake((kScreenW-titleLabelSize.width)/2, yOffset+10, titleLabelSize.width, titleLabelSize.height)
...
+ (void)initialize
{
	if (self == [TWDefaultMessageBarStyleSheet class])
	{
        ...
        kTWDefaultMessageBarStyleSheetInfoBackgroundColor = RGBA(90, 156, 248, kTWMessageBarStyleSheetMessageBarAlpha); // blue
