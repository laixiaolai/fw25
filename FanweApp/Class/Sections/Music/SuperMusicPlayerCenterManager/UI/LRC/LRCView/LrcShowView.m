...
//NSLog(@"eqeq----------------------%f -------%f -----------%f",currentTime,totalTime,present);
    ...
    dispatch_source_set_event_handler(timer, ^{
       ...
       //
        ...
        weak_Self.musicTimeCountDownLab.text = [NSString stringWithFormat:@"-%02d:%02d",(int)((totalTime-currentTime)/60)%60,(int)(totalTime-currentTime)%60];
...
- (void)showCurrentTime:(CGFloat)currentTime  musicTotalTime:(CGFloat)totalTime present:(CGFloat)present{
   // NSLog(@"eqeq----------------------%f -------%f -----------%f",currentTime,totalTime,present);
    // 默认歌词小于十行的，显示 歌手+歌曲 已经够意思了
    if (self.lrcModelMArray.count<10) {
        return;
