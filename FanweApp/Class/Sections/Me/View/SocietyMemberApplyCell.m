...
@interface SocietyMemberApplyCell()
...
@implementation SocietyMemberApplyCell
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
   static NSString *ID = @"SocietyMemberApplyCell";
   SocietyMemberApplyCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([SocietyMemberApplyCell class]) owner:nil options:nil] lastObject];
    }
    return cell;
}
...
- (IBAction)clickAgreeBtn:(UIButton *)sender {
    //同意加入公会，做删除操作
    if (_delegate && [_delegate respondsToSelector:@selector(agreeWithSocietyMemberApplyCell:)]) {
        [_delegate agreeWithSocietyMemberApplyCell:self];
    }
...
- (IBAction)clickRefuseBtn:(UIButton *)sender {
    //拒绝加入公会，做删除操作
    if (_delegate && [_delegate respondsToSelector:@selector(refuseWithSocietyMemberApplyCell:)]) {
        [_delegate refuseWithSocietyMemberApplyCell:self];
    }
}
