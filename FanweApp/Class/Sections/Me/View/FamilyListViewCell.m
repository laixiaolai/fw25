...
@interface FamilyListViewCell()
@property (nonatomic, strong) UIImageView * headView;            //家族头像
@property (nonatomic, strong) UILabel * familyNameLabel;         //家族名称
@property (nonatomic, strong) UILabel * headerNameLabel;     //家族族长名称
...
//    [_headView sd_setImageWithURL:[NSURL URLWithString:model.family_logo]];
    [_headView sd_setImageWithURL:[NSURL URLWithString:model.family_logo] placeholderImage:kDefaultPreloadHeadImg];
    _familyNameLabel.text = model.family_name;
    _headerNameLabel.text = [NSString stringWithFormat:@"族长 : %@",model.nick_name];
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(applyWithFamilyListViewCell:)]) {
        [_delegate applyWithFamilyListViewCell:self];
    }
}
