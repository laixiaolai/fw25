...
@interface EditSocietyViewController ()<UITextViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,MBProgressHUDDelegate,UINavigationControllerDelegate,OssUploadImageDelegate>
{
...
@implementation EditSocietyViewController
...
-(void)createView
{
    ...
    self.navigationItem.title = @"公会资料修改";
...
-(void)createDes
{
    ...
    else if (_type==1)
    {
        ...
        [_headBtn sd_setImageWithURL:[NSURL URLWithString:self.sociaHead] forState:UIControlStateNormal placeholderImage:kDefaultPreloadHeadImg];
        _nameTextView.text = self.sociatyName;
        //如果是未通过审核则可以编辑公会名称
        _nameTextView.editable = YES;
        ...
        _desTextView.text = self.sociatyManifasto;
...
{
    if ([_nameTextView.text isEqualToString:@"请输入公会名称"]&&[_nameTextView.textColor isEqual:[UIColor lightGrayColor]]) {
        [[FWHUDHelper sharedInstance] tipMessage:@"请填写公会名称"];
        return;
    }
    if ([_desTextView.text isEqualToString:@"请输入公会宣言"]&&[_desTextView.textColor isEqual:[UIColor lightGrayColor]]) {
        [[FWHUDHelper sharedInstance] tipMessage:@"请填写公会宣言"];
        return;
    }
    ...
    if (_type==0) {
        ...
        [mDict setObject:@"society_app" forKey:@"ctl"];
        [mDict setObject:@"create" forKey:@"act"];
        [mDict setObject:_urlStr forKey:@"logo"];
        [mDict setObject:_nameTextView.text forKey:@"name"];
        [mDict setObject:_desTextView.text forKey:@"manifesto"];
        [[FWHUDHelper sharedInstance] syncLoading:@"正在创建公会请等待"];
    ...
    else if (_type==1) // 如果是修改家族资料
    {
        ...
        NSMutableDictionary *mDict = [NSMutableDictionary dictionary];
        [mDict setObject:@"society_app" forKey:@"ctl"];
        //        [mDict setObject:@"society" forKey:@"ctl"];
        [mDict setObject:@"save" forKey:@"act"];
        [mDict setObject:[NSNumber numberWithInt:self.sociatyID] forKey:@"id"];
        if(_urlStr!=nil)
        {
            [mDict setObject:_urlStr forKey:@"logo"];
        }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    {
        NSString *nameImage = [NSString stringWithFormat:@"societyHeadImage.jpg"];
...
- (void)pictureToNet:(NSURL *)pictureUrl
{
    ...
    NSString * photoName=[imageFile stringByAppendingPathComponent:@"societyHeadImage.jpg"];
...
-(void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    else if (textView==_desTextView)
    {
        if (_desTextView.text.length<1) {
            _desTextView.text = @"请输入公会宣言";
            _desTextView.textColor = [UIColor lightGrayColor];
            [[FWHUDHelper sharedInstance] tipMessage:@"请输入公会宣言"];
            _ensureBtn.enabled = NO;
            _ensureBtn.backgroundColor = kAppGrayColor3;
        }
        else
...
{
    if (_desTextView.text.length > 140) {
        _desTextView.text = [_desTextView.text substringWithRange:NSMakeRange(0, 140)];
        [[FWHUDHelper sharedInstance] tipMessage:@"超过最大字数限制"];
    }
    return YES;
}
