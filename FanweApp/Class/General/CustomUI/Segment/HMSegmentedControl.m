...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.type == HMSegmentedControlTypeText)
    {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            else
            {
                ...
                if (idx < self.segmentWidthsArray.count)
                {
    ...
    else if (self.type == HMSegmentedControlTypeTextImages)
    {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            {
                if (self.imagePosition == HMSegmentedControlImagePositionBehindText || self.imagePosition == HMSegmentedControlImagePositionLeftOfText || self.imagePosition == HMSegmentedControlImagePositionRightOfText)
                {
                    imageHeight = self.frame.size.height - 10;
                    imageWidth = imageHeight * icon.size.width / icon.size.height;
                }
                else if (self.imagePosition == HMSegmentedControlImagePositionAboveText || self.imagePosition == HMSegmentedControlImagePositionBelowText)
                {
                    imageHeight = self.frame.size.height - stringHeight;
                    imageWidth = imageHeight * icon.size.width / icon.size.height;
                }
            }
            ...
            else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic || self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamicFixedSuperView)
            {
                // When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
                CGFloat xOffset = 0;
                NSInteger i = 0;
...
- (CGRect)frameForSelectionIndicator
{
    ...
    else
    {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            self.segmentWidthStyle != HMSegmentedControlSegmentWidthStyleDynamicFixedSuperView)
        {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
        ...
        else
        {
            ...
            else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamicFixedSuperView)
            {
                if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe)
                {
                    CGFloat widthToStartOfSelectedIndex = 0;
                    CGFloat widthToEndOfSelectedSegment = 0;
                    ...
                    for (int i = 0; i < self.segmentWidthsArray.count; i++)
                    {
                        if (i < self.selectedSegmentIndex)
                        {
                            ...
                            widthToStartOfSelectedIndex += [width floatValue];
                        }
                        else
                        {
                            break;
                    ...
                    NSNumber *width = [self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex];
                    widthToEndOfSelectedSegment = [width floatValue] + widthToStartOfSelectedIndex;
...
{
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic || self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamicFixedSuperView)
    {
        CGFloat selectedSegmentOffset = 0.0f;
...
- (void)updateSegmentsRects
{
    ...
    else if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamicFixedSuperView)
    {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
        __block CGFloat totoalStringWidth = 0;
        
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            
            totoalStringWidth += stringWidth;
            
            [mutableSegmentWidths addObject:[NSNumber numberWithFloat:stringWidth]];
            
        }];
        
        if (totoalStringWidth < self.frame.size.width)
        {
            CGFloat moreSpaceWidth = (self.frame.size.width-totoalStringWidth)/[mutableSegmentWidths count];
            
            [mutableSegmentWidths enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                
                CGFloat tmpWidth = [(NSNumber *)obj floatValue];
                tmpWidth += moreSpaceWidth;
                [mutableSegmentWidths replaceObjectAtIndex:idx withObject:[NSNumber numberWithFloat:tmpWidth]];
                
            }];
            
            //            [mutableSegmentWidths removeAllObjects];
            //
            //            CGFloat stringWidth = self.frame.size.width / self.sectionTitles.count;
            //
            //            [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            //
            //                [mutableSegmentWidths addObject:[NSNumber numberWithFloat:stringWidth]];
    ...
    else if (self.type == HMSegmentedControlTypeTextImages && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic)
    {
        ...
        if (self.shouldStretchSegmentsToSuperViewSize && totalWidth < self.bounds.size.width)
        {
    ...
    else if (self.type == HMSegmentedControlTypeTextImages && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamicFixedSuperView)
    {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
        __block CGFloat totalWidth = 0.0;
...
- (NSUInteger)sectionCount
{
    ...
    else if (self.type == HMSegmentedControlTypeImages || self.type == HMSegmentedControlTypeTextImages)
    {
