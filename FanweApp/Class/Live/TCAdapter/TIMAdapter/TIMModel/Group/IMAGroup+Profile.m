...
- (void)asyncModifyGroupName:(NSString *)name succ:(TIMSucc)succ fail:(TIMFail)fail
{
    ...
    {
        [[FWHUDHelper sharedInstance] tipMessage:@"与当前群名一致"];
        return;
    }
    
    if ([name utf8Length] > 30)
    {
        [[FWHUDHelper sharedInstance] tipMessage:@"群名称过长"];
        return;
    }
...
//    }
    ...
    if ([name utf8Length] > 30)
    {
        [[FWHUDHelper sharedInstance] tipMessage:@"群名片过长"];
        return;
    }
...
- (void)asyncModifyGroupIntroduction:(NSString *)name succ:(TIMSucc)succ fail:(TIMFail)fail
{
    ...
    {
        [[FWHUDHelper sharedInstance] tipMessage:@"与当前群介绍一致"];
        return;
    }
    
    if ([name utf8Length] > 120)
    {
        [[FWHUDHelper sharedInstance] tipMessage:@"群公告过长"];
        return;
    }
...
- (void)asyncModifyGroupNotify:(NSString *)name succ:(TIMSucc)succ fail:(TIMFail)fail
{
    ...
    {
        [[FWHUDHelper sharedInstance] tipMessage:@"与当前群公告一致"];
        return;
    }
