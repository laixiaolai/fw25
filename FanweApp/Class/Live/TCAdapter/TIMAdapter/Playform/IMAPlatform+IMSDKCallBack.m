    ...
    if (!kIsAlertingForceOffline)
    {
        ...
        [FanweMessage alert:@"下线通知" message:@"您的帐号于另一台手机上登录，是否重新登录？" destructiveAction:^{
            ...
            [self login:self.host.loginParm succ:^{
                ...
                [[FWHUDHelper sharedInstance] tipMessage:IMALocalizedError(code, msg) delay:1.0];
                [[AppDelegate sharedAppDelegate] enterLoginUI];
            }];
...
- (void)onReConnFailed:(int)code err:(NSString*)err
{
    ...
    FWWeakify(self)
    [FanweMessage alert:@"下线通知" message:@"断线重连失败，是否重新登录？" destructiveAction:^{
...
- (void)OnRefreshTicketSuccess:(TLSUserInfo *)userInfo
{
    ...
    [[TIMManager sharedInstance] login:param succ:^{
        ...
        [[FWHUDHelper sharedInstance] tipMessage:@"刷新票据，登录失败"];
    }];
}
...
- (void)OnRefreshTicketFail:(TLSErrInfo *)errInfo
{
    ...
    [[FWHUDHelper sharedInstance] syncLoading:@"刷新票据失败,正在退出"];
