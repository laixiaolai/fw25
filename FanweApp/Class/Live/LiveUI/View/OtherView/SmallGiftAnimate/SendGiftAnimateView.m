...
@interface SendGiftAnimateView()<ShakeLabelDelegate>{
...
@implementation SendGiftAnimateView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _titleNameLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(_headImgView.frame)+5, 0, frame.size.width*0.48, _backGroundView.frame.size.height/2)];
        _titleNameLabel.textColor = [UIColor whiteColor];
        _titleNameLabel.numberOfLines = 1;
        ...
        _giftNameLabel.numberOfLines = 1;
        ...
        _numLabel = [[ShakeLabel alloc]initWithFrame:CGRectMake(frame.size.width*kBackGroundWidthRate, 0, frame.size.width*(1-kBackGroundWidthRate), frame.size.height*kBackGroundHeightRate)];
...
- (void)setContent:(CustomMessageModel *)customMessageModel
{
    ...
    else if (customMessageModel.type == 28)
    {
        ...
        _numLabel.borderColor = [UIColor redColor];
    }
}
...
- (void)displayAfter
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(giftAnimate:)])
    {
        [_delegate giftAnimate:self];
    }
}
